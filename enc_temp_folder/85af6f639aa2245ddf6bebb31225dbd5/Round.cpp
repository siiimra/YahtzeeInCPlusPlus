...
void Round::reRoll(string name, vector<bool>& rerolled) {
	...
	// variable that holds the human player's choice -- y: to roll the specific dice again, n: keep the dice 
	char choice;

	// For loop to run through our 5 dice
	for (int i = 0; i < 5; i++) {

		// Check if this dice was previously rerolled. If not, skip it.
		if (!rerolled[i]) {
			cout << "\nDice " << i + 1 << " (" << diceRoll[i] << ") was not re-rolled previously, so it cannot be re-rolled again." << endl;
			continue;
		}

		// Ask user if they would like to change the current dice
		...
		if (choice == 'n' || choice == 'N') {
			...
			// Ask if they would like to input their own dice or generate randomly
			...
			if (choice == 'y' || choice == 'Y') {
				// Variable that holds custom dice value from human input
				int customDice;
				...
				// Input validation if dice is less than 1 or greater than 6
				while (customDice < 1 || customDice > 6) {
					cout << "Dice values can only range from 1-6. Try again." << endl;
					cin >> customDice;
				}
			...
			// Mark this dice as "rerolled" for the future (keeping it true)
			rerolled[i] = true;
		}
	}
